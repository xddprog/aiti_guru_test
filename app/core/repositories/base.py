...
class SqlAlchemyRepository[ModelType](RepositoryInterface[ModelType]):
    ...
    async def get_all_items(self, limit: int, offset: int) -> list[ModelType]:
        query = select(self.model)
        items: Result = await self.session.execute(query)
        return items.scalars().unique().all()

    async def get_by_attribute(self, attribute: str, value: Any, *, one_or_none: bool = False) -> ModelType | list[ModelType] | None:
    ...
    async def update_item(self, item_id: int, **update_values) -> ModelType:
        ...
        item = item.scalars().unique().first()
        await self.session.commit()
        await self.session.refresh(item)
        return item

    async def refresh(self, item: ModelType) -> None:
        await self.session.refresh(item)    
